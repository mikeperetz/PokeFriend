    ...
    if (myfile.is_open()){
       //while ( !myfile.eof() ){                                              //temporarily commented out for debugging. Only cycles through one pokemon
          ...
          while(myfile.peek() != '+'){
              ...
              string _ev_spread = statspread_probability.substr(i+1, j); cout << _ev_spread << endl;
              double _probability = stod(statspread_probability.substr(j+1, statspread_probability.length())); cout << statspread_probability << endl;
